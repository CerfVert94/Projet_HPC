...
void ui8matrix_dilation_LU3x3_O1xO3 (uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	...
	switch(r) {
		...
		case 1:
			out_row0 = ppOutput[row + 0];
			for (row = nrl; row < nrh + 1; row ++) {
				out_row0[col + 0] = scalar_or3(ppInput[row - 1], col) | 
											 scalar_or3(ppInput[row + 0], col) | 
											 scalar_or3(ppInput[row + 1], col);
			}
			break;
...
void ui8matrix_dilation_LU3x3_O3xO3_RR(uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	...
	if (nrh - nrl > 3) {
		row0 = ppInput[row - 1]; x0  = row0[col - 1]; x1  = row0[col + 0]; 
		row1 = ppInput[row + 0]; x5  = row1[col - 1]; x6  = row1[col + 0]; 
		row2 = ppInput[row + 1]; x10 = row2[col - 1]; x11 = row2[col + 0]; 
		row3 = ppInput[row + 2]; x15 = row3[col - 1]; x16 = row3[col + 0]; 
		row4 = ppInput[row + 3]; x20 = row4[col - 1]; x21 = row4[col + 0]; 
	}
...
void ui8matrix_dilation_LU3x3_O3xO3_RR_ver1(uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	...
	for (row = nrl; row < nrh + 1 - rr; row += order) {
		...
		for (col = ncl; col < nch + 1 - cr; col += order) {
			...
			// printf("%ld %ld :\n",row, col);
			// // printf("r0c0 = %u; r0c1 = %u; r0c2 = %u;\n", r0c0, r0c1, r0c2);
			// // printf("r1c0 = %u; r1c1 = %u; r1c2 = %u;\n", r1c0, r1c1, r1c2);
			// // printf("r2c0 = %u; r2c1 = %u; r2c2 = %u;\n", r2c0, r2c1, r2c2);
			// printf("r0c0 = %u %u %u %u %u %u %u %u %u \n", x0  , x1  , x2  , x5  , x6  , x7  , x10 , x11 , x12);
			// printf("r0c1 = %u %u %u %u %u %u %u %u %u \n", x1  , x2  , x3  , x6  , x7  , x8  , x11 , x12 , x13);
			// printf("r0c2 = %u %u %u %u %u %u %u %u %u \n", x2  , x3  , x4  , x7  , x8  , x9  , x12 , x13 , x14);
			// printf("r1c0 = %u %u %u %u %u %u %u %u %u \n", x5  , x6  , x7  , x10 , x11 , x12 , x15 , x16 , x17);
			// printf("r1c1 = %u %u %u %u %u %u %u %u %u \n", x6  , x7  , x8  , x11 , x12 , x13 , x16 , x17 , x18);
			// printf("r1c2 = %u %u %u %u %u %u %u %u %u \n", x7  , x8  , x9  , x12 , x13 , x14 , x17 , x18 , x19);
			// printf("r2c0 = %u %u %u %u %u %u %u %u %u \n", x10 , x11 , x12 , x15 , x16 , x17 , x20 , x21 , x22);
			// printf("r2c1 = %u %u %u %u %u %u %u %u %u \n", x11 , x12 , x13 , x16 , x17 , x18 , x21 , x22 , x23);		
			// printf("r2c2 = %u %u %u %u %u %u %u %u %u \n", x12 , x13 , x14 , x17 , x18 , x19 , x22 , x23 , x24);
...
void ui8matrix_dilation_LU3x3_O1xO3_RR (uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	...
	for (row = nrl; row < nrh + 1; row++) {
		...
		for (col = ncl; col < nch + 1 - r; col += order) {
			...
			y1 = x1 | x2 | x6 | x7 | x11 | x12; // 5 calcs
			out_row0[col + 0] = y1 | x0 | x5 | x10; // 3 calc	
			out_row0[col + 1] = y1 | x3 | x8 | x13; // 3 calc	
			out_row0[col + 2] =     (x2 | x3 | x4) | (x8 | x7 | x9) | (x12 | x13 | x14); //8 calc 
...
void ui8matrix_dilation_LU3x3_O3xO3_RR(uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	...
	for (row = nrl; row < nrh + 1 - rr; row += order) {
		...
		x5  = row1[-1]; x6  = row1[ 0]; 
		x10 = row2[-1]; x11 = row2[ 0]; 
		x15 = row3[-1]; x16 = row3[ 0]; 
		x20 = row4[-1]; x21 = row4[ 0]; 
		for (col = ncl; col < nch + 1 - cr; col += order) {
