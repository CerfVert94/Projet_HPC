...
void ui8matrix_dilation_LU3x3_InLU_O3_AddrRR_OMP(uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	...
	omp_set_num_threads(omp_get_max_threads());
	#pragma omp parallel default(none) firstprivate(nrl, nrh) private(out_row0, row0, row1, row2, row3, row4, y0, y1, y2, x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, row, col) shared(ppInput, ncl, nch, ppOutput, nb_threads, r, nrow)
	{
...
void ui8matrix_dilation_LU3x3_ExLU_O3_AddrRR_OMP(uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	...
	nrow = nrh - nrl + 1;
	omp_set_num_threads(omp_get_max_threads());
	#pragma omp parallel default(none) firstprivate(nrl, nrh) private(out_row0,out_row1, out_row2, row0, row1, row2, row3, row4,y0, y1, y2, y3, y5, x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, row, col) shared(ppInput, ncl, nch, ppOutput, nb_threads, r, nrow)
	{
...
void ui8matrix_dilation_LU3x3_ComLU_O3_AddrRR_OMP(uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	...
	nrow = nrh - nrl + 1;
	omp_set_num_threads(omp_get_max_threads());
	#pragma omp parallel default(none) firstprivate(nrl, nrh) private(out_row0, out_row1, out_row2, row0, row1, row2, row3, row4, row, col) shared(ppInput, ncl, nch, ppOutput, nb_threads, cr, rr, nrow)
	{
...
void ui8matrix_dilation_divide_row_and_conquer_InLU_O3_OMP (uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	...
	for (row = nrl; row < nrh + 1; row ++)
	{
		...
		for(col = ncl; col < nch + 1 - r; col += order) {
			out_row0[col + 0] = temp_row0[col + 0] |
								temp_row1[col + 0] |
								temp_row2[col + 0];
			out_row0[col + 1] = temp_row0[col + 1] |
								temp_row1[col + 1] |
								temp_row2[col + 1];
			out_row0[col + 2] = temp_row0[col + 2] |
								temp_row1[col + 2] |
								temp_row2[col + 2];
		}
		
		switch(r){
			case 2: out_row0[nch - 1] = temp_row0[nch - 1] | temp_row1[nch - 1] | temp_row2[nch - 1];
			case 1: out_row0[nch + 0] = temp_row0[nch + 0] | temp_row1[nch + 0] | temp_row2[nch + 0];
			case 0: break;
		}
	}
...
void ui8matrix_dilation_divide_col_and_conquer_OMP (uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	...
	for (row = nrl; row < nrh + 1; row ++)
	{
		...
		in_row1   = ppInput[row + 0];
		in_row2   = ppInput[row + 1];
		for(col = ncl + 0; col < nch + 1; col+= order) 
			temp_row[col] = in_row0[col] | in_row1[col] | in_row2[col];
		for(col = ncl - 1; col < nch + 1; col+= order) 
			temp_row[col] = in_row0[col] | in_row1[col] | in_row2[col];
		for(col = ncl + 1; col < (nch + 1) + 1; col+= order) 
			temp_row[col] = in_row0[col] | in_row1[col] | in_row2[col];
		temp_row[nch + 1] = in_row0[nch + 1] | in_row1[nch + 1] | in_row2[nch + 1];
	}
...
void ui8matrix_dilation_LU3x3_ExLU_O3_NS_OMP(uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	const long order = 3;
	long row = nrl, col = ncl, x, y;
	uint8 y0, y1, y2, x0, x1, x2, x3, r;
...
void ui8matrix_dilation_LU3x3_InLU_O3_NS_OMP(uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	const long order = 3;
	long row = nrl, col = ncl, x, y, r;
	uint8 y0, y1, y2, x0, x1, x2, x3, x4, x5, x6, x7, x8;
...
void ui8matrix_dilation_LU3x3_ComLU_O3_NS_OMP(uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	const long order = 3;
	long row = nrl, col = ncl, x, y;
	uint8 y0, y1, y2, rr, cr;
...
void ui8matrix_dilation_LU3x3_ExLU_O3_RR_NS_OMP(uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	const long order = 3;
	long row = nrl, col = ncl, x, y;
	uint8 y0, y1, y2, r;
...
void ui8matrix_dilation_LU3x3_InLU_O3_RR_NS_OMP(uint8** ppInput, long nrl, long nrh, long ncl, long nch, p_struct_elem_dim s, uint8 **ppOutput)
{
	const long order = 3;
	long row = nrl, col = ncl, x, y, r;
